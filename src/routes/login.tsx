...
import { Sparkle, Eye, EyeOff, ArrowRight, Mail, Lock } from "lucide-react";

export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      { title: "MIRROR — Log In" },
      { name: "description", content: "Log in to your MIRROR creative twin dashboard." },
    ],
  }),
  component: LoginPage,
});
    ...
    <div className="min-h-screen flex relative">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div
          ...
          className="absolute top-[15%] left-[8%] size-32 rounded-3xl border border-violet/15 bg-violet/5 backdrop-blur-sm"
          style={{ transform: "perspective(800px) rotateX(15deg) rotateY(-10deg)" }}
        />
        <motion.div
          animate={{ y: [0, 12, 0], rotateZ: [0, -6, 0] }}
          transition={{ duration: 6, repeat: Infinity, ease: "easeInOut", delay: 1 }}
          className="absolute bottom-[20%] right-[10%] size-24 rounded-full border border-cyan/15 bg-cyan/5 backdrop-blur-sm"
        />
        <motion.div
          animate={{ y: [0, -10, 0] }}
          transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: 2 }}
          className="absolute top-[60%] left-[20%] size-16 rounded-xl border border-violet/10 bg-violet/3"
        ...
        <div className="max-w-md">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
            ...
            <p className="text-muted-foreground leading-relaxed">
              Your AI alter ego is waiting. Pick up where you left off — your Style DNA, content pipeline, and creative twin are exactly as you left them.
            </p>
            ...
            <div className="mt-10 glass rounded-xl p-5">
              ...
              <div className="text-sm font-medium">RAG Index online · 12.4M creator signals</div>
              <div className="text-[11px] text-muted-foreground mt-1">Style DNA engine ready</div>
            </div>
          </motion.div>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12 relative z-10">
        ...
        >
          ...
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight mb-1">Log in</h1>
          <p className="text-sm text-muted-foreground mb-8">
            Don't have an account?{" "}
            <Link to="/signup" className="text-violet hover:text-violet/80 font-medium transition">
              Sign up
            </Link>
          </p>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 text-xs text-muted-foreground cursor-pointer">
                <input type="checkbox" className="size-3.5 rounded border-white/20 bg-white/5 accent-violet" />
                Remember me
              </label>
              <a href="#" className="text-xs text-violet hover:text-violet/80 transition">Forgot password?</a>
            </div>
          ...
          <div className="mt-8 relative">
            ...
            <div className="relative flex justify-center"><span className="bg-[var(--background)] px-3 text-xs text-muted-foreground">or continue with</span></div>
          </div>
